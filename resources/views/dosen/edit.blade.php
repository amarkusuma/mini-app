        ...
        <div class="form-row">
            <div class="col-lg-6 col-md-6 mb-2">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="name" value="{{$dosen->name}}" id="name" placeholder="Name" required>
                </div>
            </div>
            <div class="col-lg-6 col-md-6 mb-2">
                <div class="form-group">
                    <label for="name">Foto</label>
                    <input type="file" class="form-control" name="foto" value="" id="name" placeholder="Upload Foto" accept=".png,.jpg,.jpeg">
                </div>
            </div>
            <div class="col-lg-6 col-md-6 mb-2">
                <div class="form-group">
                    ...
                    <select class="form-control status" name="status_id" id="status" required>
                        ...
                        @foreach ($status as $key => $item)
                            <option id="{{$item->id}}" value="{{ $item->id }}" @if (old('status_id', isset($dosen) ? $dosen->status_id : '') == $item->id) selected @endif  >{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control fakultas" name="fakultas_id" id="fakultas" required>
                        ...
                        @foreach ($fakultas as $key => $item)
                            <option id="{{$item->id}}" value="{{ $item->id }}" @if (old('fakultas_id', isset($dosen) ? $dosen->fakultas_id : '') == $item->id) selected @endif>{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control jurusan" name="jurusan_id" id="jurusan" required>
                        ...
                        @foreach ($jurusan as $key => $item)
                            <option id="{{$item->id}}" value="{{ $item->id }}" @if (old('jurusan_id', isset($dosen) ? $dosen->jurusan_id : '') == $item->id) selected @endif>{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="col-lg-6 col-md-6 mb-2">
                <div class="form-group">
                    ...
                    <textarea type="text" class="form-control" name="address" id="address" placeholder="Address" required rows="3">{{$dosen->address}} </textarea>
                </div>
            </div>
            <div class="col-lg-6 col-md-6 mb-2">
                <div class="form-group">
                    ...
                    <select class="form-control province" name="prov_id" id="province" required>
                        ...
                        @foreach ($province as $key => $item)
                            <option id="{{$item->prov_id}}" value="{{ $item->prov_id }}" @if (old('prov_id', isset($dosen) ? $dosen->prov_id : '') == $item->prov_id) selected @endif >{{ $item->prov_name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="col-lg-6 col-md-6 mb-2">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" value="{{$dosen->Rt}}" name="Rt" id="Rt" placeholder="Rt" required>
                </div>
            </div>
        </div>
...
<script>
    const prov_id = "{{ old('prov_id', isset($dosen) ? $dosen->prov_id : '') }}";
    const city_id = "{{ old('city_id', isset($dosen) ? $dosen->city_id : '') }}";
    const district_id = "{{ old('dis_id', isset($dosen) ? $dosen->dis_id : '') }}";
    const subdistrict_id = "{{ old('subdis_id', isset($dosen) ? $dosen->subdis_id : '') }}";
    ...
    $.ajax({
        ...
        $.ajax({
            ...
            $.ajax({
                type: 'GET',
                url: `/subdistricts/${district_id}`,
                data: {
                    'dis_id': district_id
                },
                beforeSend: function () {
                    $('#subdistrict').empty();
                    $('#subdistrict').html('<option value="" disabled>Pilih Desa</option>');
                },
                success: function (data) {
                    // console.log(data);
                    ...
                    for (var i = 0; i < data.length; i++) {
                        var selected =  (subdistrictId == data[i].subdis_id) ? 'selected' : '';
